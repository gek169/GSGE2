...
// We will need to be adding more uniform variable types as time goes on,
// especially arrays of data e.g. arrays of vec4s and arrays of vec3s

// Stuff to do with setting uniforms
void Shader::setUniform1f(std::string name, GLfloat value) { glUniform1f(getUniformLocation(name), value); } // 1 float
...
void Shader::setUniform1i(std::string name, GLint value) { glUniform1i(getUniformLocation(name), value); } // Integer

void Shader::setUniform1ui(std::string name, GLuint value) { glUniform1ui(getUniformLocation(name), value); } // Unsigned Integer
// Deprecated
/*
void Shader::Update(Transform inTransform, Camera thiscamera)
{
		...
		glUniformMatrix4fv(m_uniforms[TRANSFORM_UNIFORM], 1, GL_FALSE,
&model[0][0]); glUniformMatrix4fv(m_uniforms[MVP_UNIFORM], 1, GL_FALSE,
&camshit[0][0]);
}
*/
}; // namespace gekRender
